...
class NBNPubIdPlugin extends PubIdPlugin {
   ...
   function getResolvingURL($contextId, $pubId) {
	...
	function getLinkActions($pubObject) {
		...
		//var_dump($pubObject);
		//if(!$pubObject) return;
		$contextId = $pubObject->getPressId();
	...
	function getSuffixPatternsFieldNames() {
		return  array(
			...
			'SubmissionFile' => 'nbnSubmissionFileSuffixPattern',
		);
	}
	...
	*/   
	function getPubId($pubObject){
		...
		$journalId = $pubObject->getPressId();
    ...
    */   
	function generateNBN($journal, $articleId){
		...
		$articleDao = DAORegistry::getDAO('PublishedMonographDAO'); 
		
		$article = $articleDao->getById($articleId, null, true);	
		//var_dump($articleDao);
		$request = Application::getRequest();
		$router = $request->getRouter();
   ...
   /*
    * Check if the submission is published 
    * 
    * */
